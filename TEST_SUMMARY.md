...
│   ├── test_kafka_connect.py      # 28 tests
...
**Tests**: 6 in `TestKafkaConnectService`
...
- `test_connect_service_depends_on_kafka` - Validates dependency with health check
- `test_connect_service_environment_variables` - Verifies all 11 required env vars
- `test_connect_service_exposes_rest_api_port` - Confirms port 8083 exposure
...
**Tests**: 16 in `TestJdbcSinkConnectorConfiguration`
...
- `test_air_connector_uses_environment_variables_for_connection` - Uses ${env:PG_*}
...
- `test_weather_connector_uses_environment_variables_for_connection` - Uses ${env:PG_*}
...
1. **Fast Feedback**: All 53 tests run in ~0.3 seconds
